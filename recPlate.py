...
def pre_processing(img,device,img_sz=(640,640)):  #前处理
    img, r,left,top= letter_box(img,(img_sz[0],img_sz[1]))
    # print(img.shape)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--detect_model', nargs='+', type=str, default=r'weights/yolo11s-plate-detect.pt', help='model.pt path(s)')  #yolov11检测模型
    parser.add_argument('--rec_model', type=str, default=r'weights/plate_rec_color.pth', help='model.pt path(s)')#车牌字符识别模型  
    parser.add_argument('--image_path', type=str, default=r'1', help='source')   #待识别图片路径
    parser.add_argument('--img_size', type=int, default=640, help='inference size (pixels)')  #yolov11 网络模型输入大小
